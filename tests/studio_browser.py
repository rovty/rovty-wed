...
with sync_playwright() as p:
    ...
    expect(page.get_by_role('button',name=re.compile('^Preview '))).to_have_count(14)
    page.get_by_role('button',name='Cultural',exact=True).click()
    expect(page.get_by_role('button',name=re.compile('^Preview '))).to_have_count(4)
    page.get_by_role('button',name='All designs',exact=True).click()
    page.get_by_role('textbox',name='Search designs').fill('noir')
    expect(page.get_by_role('button',name=re.compile('^Preview '))).to_have_count(1)
    page.get_by_role('button',name='Clear search').click()
    page.get_by_role('button',name='Compare Editorial',exact=True).click()
    page.get_by_role('button',name='Compare Garden',exact=True).click()
    ...
    frame = page.frame_locator('iframe[title="Editorial live preview"]')
    ...
    page.get_by_role('button',name='Switch to Noir',exact=True).click()
    expect(canvas.get_by_role('heading',level=1)).to_contain_text('Nila')
    expect(canvas.get_by_text('We met under a summer sky.',exact=True)).to_have_count(1)
    expect(canvas.get_by_role('timer')).to_have_count(0)
    page.get_by_role('button',name='Undo',exact=True).click()
    expect(canvas.locator('.hero-editorial')).to_have_count(1)
    page.get_by_role('button',name='Redo',exact=True).click()
    expect(canvas.locator('.hero-noir')).to_have_count(1)
    ...
    page.get_by_role('button',name='Designs',exact=True).click()
    for name in ['Editorial','Garden','Noir','Quiet','Poruwa','Bloom','Film','Thali','Shoreline','Deco','Chapel','Nikkah','Lotus','Classic']:
    ...
    print('PASS: discovery, filters, compare, preview RSVP, live editing without reload, palettes, fonts, section visibility, content retention, undo/redo, upload, draft restoration and all 14 mobile templates.')
    context.close()
    browser.close()
